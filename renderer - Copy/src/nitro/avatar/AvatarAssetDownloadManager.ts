...
export class AvatarAssetDownloadManager extends EventDispatcher
{
    ...
    private processFigureMap(data: any): void
    {
        ...
        for(const library of data)
        {
            ...
            const assetUrls = [
                NitroConfiguration.getValue<string>('avatar.asset.url'),
                NitroConfiguration.getValue<string>('alternative.avatar.asset.url')
            ].filter((value): value is string => !!value);

            const downloadLibrary = new AvatarAssetDownloadLibrary(id, revision, this._assets, assetUrls);
