...
export class RoomContentLoader implements IFurnitureDataListener, IRoomContentLoader
{
    ...
    public async downloadAsset(type: string, events: IEventDispatcher): Promise<void>
    {
        ...
        {
            if(!assetUrl || !assetUrl.length) continue;
            ...
            try
            {
                ...
                if(!response.ok)
                {
                    NitroLogger.warn(`Failed to fetch furni asset from ${ assetUrl } (status ${ response.status })`);
                ...
                switch(contentType)
                {
                    case 'application/octet-stream': {
                        ...
                        const dispatcher = this._events.get(type);

                        if(!dispatcher) return;

                        dispatcher.dispatchEvent(new RoomContentLoadedEvent(RoomContentLoadedEvent.RCLE_SUCCESS, type));
            ...
            {
                NitroLogger.error('Failed to download furni asset', err);
            }
        }
        ...
        switch(type)
        {
            ...
            default: {
                ...
                if((category === RoomObjectCategory.FLOOR) || (category === RoomObjectCategory.WALL))
                {
                    ...
                    const alternativeUrl = this.getAlternativeFurniAssetUrl(type);

                    if(alternativeUrl) urls.push(alternativeUrl);
    ...
    private getAlternativeFurniAssetUrl(assetName: string): string
    {
        const alternativeBase = NitroConfiguration.getValue<string>('alternative.furni.asset.url');

        if(!alternativeBase) return null;

        return alternativeBase.replace(/%libname%/gi, assetName);
    }
