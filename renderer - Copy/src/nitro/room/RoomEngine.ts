...
export class RoomEngine extends NitroManager implements IRoomEngine, IRoomCreator, IRoomEngineServices, IRoomManagerListener, IRoomContentListener, IUpdateReceiver, IDisposable
{
    ...
    public removeRoomObjectUser(roomId: number, objectId: number): void
    {
        ...
        if(instanceData) instanceData.clearUserFocusAlpha(objectId);

        this.removeRoomObject(roomId, objectId, RoomObjectCategory.UNIT);
    }
    ...
    public addRoomObjectUser(roomId: number, objectId: number, location: IVector3D, direction: IVector3D, headDirection: number, type: number, figure: string): boolean
    {
        ...
        if(instanceData)
        {
            const storedFocus = instanceData.getUserFocusAlpha(objectId);

            if((storedFocus !== null) && (storedFocus !== undefined)) focusAlpha = storedFocus;
        }

        if(object.model)
        {
            object.model.setValue(RoomObjectVariable.FIGURE_FOCUS_ALPHA, focusAlpha);
    ...
    public updateRoomObjectUserAction(roomId: number, objectId: number, action: string, value: number, parameter: string = null): boolean
    {
        ...
        if(!object)
        {
            if(action === RoomObjectVariable.FIGURE_FOCUS_ALPHA)
            {
                const clampedValue = Math.max(0, Math.min(255, value));

                if(instanceData) instanceData.setUserFocusAlpha(objectId, clampedValue);
        ...
        switch(action)
        {
            ...
            case RoomObjectVariable.FIGURE_IS_PLAYING_GAME:
                ...
                break;
            case RoomObjectVariable.FIGURE_FOCUS_ALPHA:
                if(object.model)
                {
                    const clampedValue = Math.max(0, Math.min(255, value));

                    object.model.setValue(RoomObjectVariable.FIGURE_FOCUS_ALPHA, clampedValue);
