...
import { IRoomAreaSelectionManager, IRoomEngine, IRoomObject } from "api";

import { RoomEngineObjectEvent, RoomObjectMouseEvent, RoomObjectTileMouseEvent } from "../../../../src/events/room";
...
export class RoomAreaSelectionManager implements IRoomAreaSelectionManager
{
    ...
    constructor(roomEngine: IRoomEngine)
    {
        ...
        {
            this._roomEngine.events.addEventListener(RoomEngineObjectEvent.ADDED, event =>
            {
                ...
                if(roomObject && roomObject.visualization instanceof FurnitureVisualization)
                    roomObject.visualization.lookThrough = true;
            });
        }
        else
        {
            console.warn("Room engine o su propiedad events no están disponibles.");
        }
    ...
    {
        return this._roomEngine.getRoomObjects(this._roomEngine.activeRoomId, 20)
            .concat(this._roomEngine.getRoomObjects(this._roomEngine.activeRoomId, 10));
    }
    ...
    public handleTileMouseEvent(event: RoomObjectTileMouseEvent): void
    {
        ...
        if ((this._state === RoomAreaSelectionManager.SELECTING) && (event.type === RoomObjectMouseEvent.MOUSE_MOVE))
        {
            if ((event.tileXAsInt !== this._tileXEnd) || (event.tileYAsInt !== this._tileYEnd))
            {
                ...
                let rootX = this._tileXInit < this._tileXEnd ? this._tileXInit : this._tileXEnd;
                let width = Math.abs(this._tileXEnd - this._tileXInit) + 1;
                let rootY = this._tileYInit < this._tileYEnd ? this._tileYInit : this._tileYEnd;
                let height = Math.abs(this._tileYEnd - this._tileYInit) + 1;
