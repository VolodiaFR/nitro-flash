...
export class RoomPlaneParser
{
    ...
    {
        // Special handling for highlight areas - create a single plane covering the entire selection
        if (param6) {
            this.extractHighlightPlanes(param1, param2, param3, param4, param5);
            return;
        }
    ...
    private extractHighlightPlanes(param1: number[][], param2: number = 0, param3: number = 0, param4: number = -1, param5: number = -1): void
    {
        const _loc14_: number = param1.length;
        const _loc26_: number = param1[0].length;
        const _loc27_: number = param5 == -1 ? _loc14_ : Math.min(_loc14_, param3 + param5);
        ...
        // Create a map to track which tiles are selected
        const selectedTiles: boolean[][] = [];
        for (let y = 0; y < _loc14_; y++) {
            selectedTiles[y] = [];
            for (let x = 0; x < _loc26_; x++) {
                selectedTiles[y][x] = (y >= param3 && y < _loc27_ && x >= param2 && x < _loc22_ && param1[y][x] >= 0);
            }
        }

        // Group tiles by height and create planes for each height level
        const heightGroups: Map<number, {minX: number, maxX: number, minY: number, maxY: number, tiles: {x: number, y: number}[]}> = new Map();

        for (let y = param3; y < _loc27_; y++) {
            for (let x = param2; x < _loc22_; x++) {
                const height = param1[y][x];
                if (height >= 0) {
                    ...
                    const group = heightGroups.get(height);
                    ...
                    group.tiles.push({x, y});
                }
            }
        }

        // Create highlight planes for each height group
        for (const [height, group] of heightGroups) {
            // Create a single plane covering the entire selection area for this height
            const startX = param2 / 4 - 0.5;
            const startY = param3 / 4 - 0.5;
            const width = (param4 == -1 ? _loc22_ - param2 : param4) / 4;
            const heightDiff = (param5 == -1 ? _loc27_ - param3 : param5) / 4;

            // Add a small offset to the Z position to ensure highlight renders above floor tiles
            const highlightZ = (height / 4) + 0.01;

            // Determine which edges are actually at the boundary of the selection
            const leftEdge = param2 == 0 || !this.hasValidTileAt(param1, param2 - 1, param3, _loc27_ - param3, height);
            const topEdge = param3 == 0 || !this.hasValidTileAt(param1, param2, param3 - 1, _loc22_ - param2, height);
            const rightEdge = _loc22_ == _loc26_ || !this.hasValidTileAt(param1, _loc22_, param3, _loc27_ - param3, height);
            const bottomEdge = _loc27_ == _loc14_ || !this.hasValidTileAt(param1, param2, _loc27_, _loc22_ - param2, height);

            this.addFloor(
                new Vector3d(startX + width, startY + heightDiff, highlightZ),
                new Vector3d(-width, 0, 0),
                new Vector3d(0, -heightDiff, 0),
    ...
    private hasValidTileAt(tileMatrix: number[][], startX: number, startY: number, length: number, expectedHeight: number): boolean
    {
        const maxY = tileMatrix.length;
        const maxX = tileMatrix[0].length;
        
        for (let i = 0; i < length; i++) {
            const x = startX + (startY === startY ? i : 0); // horizontal check
            const y = startY + (startX === startX ? i : 0); // vertical check
            
            if (x >= 0 && x < maxX && y >= 0 && y < maxY) {
                if (tileMatrix[y][x] === expectedHeight) {
                    return true;
                }
            }
        }
