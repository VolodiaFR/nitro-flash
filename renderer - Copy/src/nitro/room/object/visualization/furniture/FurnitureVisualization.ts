...
export class FurnitureVisualization extends RoomObjectSpriteVisualization {
    ...
    private _lookThroughCustom: boolean;
    private _needsLookThroughUpdateCustom: boolean;
    ...
    public update(geometry: IRoomGeometry, time: number, update: boolean, skipUpdate: boolean): void {
        ...
        if (this._needsLookThroughUpdateCustom) {
            updateSprites = true;
            this._needsLookThroughUpdateCustom = false;
        }
        if (this._needsLookThroughUpdateCustomVar) {
            updateSprites = true;
            this._needsLookThroughUpdateCustomVar = false;
        }
    ...
    protected updateSprite(scale: number, layerId: number): void {
        ...
        if (assetName && sprite) {
            ...
            if (assetData && assetData.texture) {
                ...
                if (this._lookThroughCustom) {
                    sprite.alpha *= 0.6;
                    sprite.color = 0x00FFFF;
                }

                if (this._lookThroughCustomVar) {
                    sprite.alpha *= 0.6;
                    sprite.color = 0x9900FF;
                }
    ...
    public set lookThroughCustom(flag: boolean) {
        if (this._lookThroughCustom == flag) return;

        this._lookThroughCustom = flag;
        this._needsLookThroughUpdateCustom = true;
    }

    public set lookThroughCustomVar(flag: boolean) {
        if (this._lookThroughCustomVar == flag) return;

        this._lookThroughCustomVar = flag;
        this._needsLookThroughUpdateCustomVar = true;
    }
}
