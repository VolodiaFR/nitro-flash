...
export class AvatarVisualization extends RoomObjectSpriteVisualization implements IAvatarImageListener, IAvatarEffectListener
{
    ...
    private static MOVE_ANIMATION_MS: number = 500; // ms per movement animation step
    ...
    private static FOCUS_BASE_ALPHA_KEY: string = '__focusBaseAlpha';
        ...
        if(update1 || update2)
        {
            ...
            for(const spriteData of this._avatarImage.getSprites())
            {
                if(spriteData.id === AvatarVisualization.AVATAR)
                {
                    ...
                    if(sprite)
                    {
                        sprite.alpha = (this._avatarImage && this._avatarImage.isPlaceholder()) ? 150 : 255;
    ...
    protected updateModel(model: IRoomObjectModel, scale: number): boolean
    {
        ...
        // If posture is walking ('mv'), set updates-per-frame to match ~500ms per animation step
        if(this._posture === 'mv')
        {
            const steps = Math.max(1, Math.ceil(AvatarVisualization.MOVE_ANIMATION_MS / AvatarVisualization.UPDATE_TIME_INCREASER));
            this._updatesUntilFrameUpdate = steps;
        }
        else
        {
            if(this._updatesUntilFrameUpdate <= 0) this._updatesUntilFrameUpdate = AvatarVisualization.ANIMATION_FRAME_UPDATE_INTERVAL;
        }

        // If posture changed to walking ('mv'), slow down frame updates to match movement (500ms)
        if(this._posture === 'mv')
        {
            const steps = Math.max(1, Math.ceil(AvatarVisualization.MOVE_ANIMATION_MS / AvatarVisualization.UPDATE_TIME_INCREASER));
            this._updatesUntilFrameUpdate = steps;
        ...
        let focusAlpha = model.getValue<number>(RoomObjectVariable.FIGURE_FOCUS_ALPHA);

        if((focusAlpha === undefined) || (focusAlpha === null)) focusAlpha = 255;

        focusAlpha = Math.max(0, Math.min(255, focusAlpha));

        if(focusAlpha !== this._focusAlpha)
        {
            this._focusAlpha = focusAlpha;
    ...
    private updateShadow(scale: number): void
    {
        ...
        if (this.object.getLocation().z > 0) hasShadow = false;
    ...
    private applyFocusAlpha(sprite: IRoomObjectSprite): void
    {
        this.enforceFocusAlphaForSprite(sprite);
    }

    private enforceFocusAlpha(): void
    {
    ...
    private enforceFocusAlphaForSprite(sprite: IRoomObjectSprite): void
    {
        if(!sprite) return;

        const spriteData = (sprite as unknown as Record<string, number | undefined>);
        const baseAlphaKey = AvatarVisualization.FOCUS_BASE_ALPHA_KEY;

        const currentAlpha = (sprite.alpha === undefined || sprite.alpha === null) ? 255 : sprite.alpha;

        if(this._focusAlpha >= 255)
        {
            const storedAlpha = spriteData[baseAlphaKey];

            if(storedAlpha !== undefined)
            {
                sprite.alpha = storedAlpha;
                delete spriteData[baseAlphaKey];
            }
            else
            {
        ...
        if(spriteData[baseAlphaKey] === undefined)
        {
            spriteData[baseAlphaKey] = currentAlpha;
        }
        else if(currentAlpha !== this._focusAlpha)
        {
            spriteData[baseAlphaKey] = currentAlpha;
        }

        const baseAlpha = (spriteData[baseAlphaKey] === undefined) ? currentAlpha : spriteData[baseAlphaKey];

        if(baseAlpha > this._focusAlpha)
        {
            sprite.alpha = this._focusAlpha;
        }
        else
        {
