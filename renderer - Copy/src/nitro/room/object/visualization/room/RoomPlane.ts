...
export class RoomPlane implements IRoomPlane
{
    ...
    private getTexture(geometry: IRoomGeometry, timeSinceStartMs: number): RenderTexture {
        ...
        if (!this._rasterizer) {
            console.warn(`Rasterizer no inicializado para el plano ${this._id}`);
            return null;
        }
        ...
        if (this.needsNewTexture(geometry, timeSinceStartMs)) {
            ...
            try {
                bitmapData = this._rasterizer.render(
                    ...
                    this._textureMaxX, this._textureMaxY,
                    timeSinceStartMs
                );
            } catch (error) {
                console.error(`Error al renderizar textura: ${error}`);
                return null;
            }
        ...
        return bitmapData?.texture || null;
    }
    ...
    public update(geometry: IRoomGeometry, timeSinceStartMs: number): boolean
    {
        ...
        if(geometryChanged)
        {
            ...
            // Apply extra depth for highlight planes (subtract to bring forward)
            relativeDepth -= this._extraDepth;
