        ...
        while (index < this._roomPlaneParser.planeCount) {
            ...
            if (location && leftSide && rightSide) {
                ...
                if (planeType === RoomPlaneData.PLANE_FLOOR) {
                    ...
                    plane.color = ((_local_14.x === 0) && (_local_14.y === 0))
    ...
    protected defineSprites(): void {
        ...
        while (planeIndex < this._planes.length) {
            ...
            if (plane && sprite && plane.leftSide && plane.rightSide) {
                // 1. Configurar tolerancia alpha para interacción
                if ((plane.type === RoomPlane.TYPE_WALL) && ((plane.leftSide.length < 1) || (plane.rightSide.length < 1))) {
                    ...
                    // Ajustar tolerancia para permitir hover
                    sprite.alphaTolerance = AlphaTolerance.MATCH_OPAQUE_PIXELS; // o AlphaTolerance.MATCH_NOTHING si usas transparencias
                }
    
                // 2. Etiquetado consistente para identificaciÃ³n
                ...
                // 3. Modificar la lÃ³gica de highlight para permitir interacciÃ³n
                if (this._roomPlaneParser.isPlaneTemporaryHighlighter(planeIndex)) {
                    if (this._highlightFilter) {
                        sprite.filters = [this._highlightFilter];
                    }
                    
                    // --- CAMBIOS CRÃTICOS AQUÃ ---
                    sprite.skipMouseHandling = false; // Permitir interacciÃ³n
                    plane.extraDepth = 100; // Asegurar renderizado encima
                    plane.isHighlighter = true;
                    
                    // Ajustar alphaTolerance especÃ­fico para highlights
                    sprite.alphaTolerance = AlphaTolerance.MATCH_NOTHING; 
                } else {
                    sprite.filters = [];
                    sprite.skipMouseHandling = false; // O ajustar segÃºn necesidad
                    plane.extraDepth = 0;
