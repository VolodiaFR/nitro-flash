...
export interface IInternalGlobalVariableData {
    name: string;
    availability: string;
    hasValue: boolean;
    canWriteTo: boolean;
...
export class RoomInternalGlobalVariablesMessageParser {
    private _vars: IInternalGlobalVariableData[] = [];

    public flush(): boolean { this._vars = []; return true; }
    ...
    public parse(wrapper: IMessageDataWrapper): boolean {
        ...
        for(let i=0;i<count;i++) {
            ...
            const wiredItemId = wrapper.readInt();
            ...
            this._vars.push({ name, availability, hasValue, canWriteTo, canCreateDelete, isAlwaysAvailable, hasCreationTime, hasUpdateTime, isTextConnected, wiredItemId, hasRuntimeValue, runtimeValue, creationTimestamp, updateTimestamp, textEntries });
        }
        return true;
    }

    public get vars(): IInternalGlobalVariableData[] { return this._vars; }
}