...
export interface IInternalFurniVariableData {
    name: string;
    availability: string;
    hasValue: boolean;
    canWriteTo: boolean;
...
export class RoomInternalFurniVariablesMessageParser {
    private _vars: IInternalFurniVariableData[] = [];
    ...
    public parse(wrapper: IMessageDataWrapper): boolean {
        ...
        for(let i=0;i<count;i++) {
            ...
            this._vars.push({ name, availability, hasValue, canWriteTo, canCreateDelete, isAlwaysAvailable, hasCreationTime, hasUpdateTime, isTextConnected, furniId, hasRuntimeValue, runtimeValue, creationTimestamp, updateTimestamp, textEntries });
        }
        return true;
    }

    public get vars(): IInternalFurniVariableData[] { return this._vars; }
}
