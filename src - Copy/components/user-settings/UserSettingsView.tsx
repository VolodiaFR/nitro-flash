...
export const UserSettingsView: FC<{}> = props =>
{
    ...
    const processAction = (type: string, value?: boolean | number | string) =>
    {
        ...
        switch(type)
        {
            ...
            case 'oldchat':
                ...
                break;
            case 'oldrotations':
                clone.oldRotations = value as boolean;
                SendMessageComposer(new UserSettingsOldRotationsComposer(clone.oldRotations));
                break;
            case 'room_invites':
    ...
    useMessageEvent<UserSettingsEvent>(UserSettingsEvent, event =>
    {
        ...
        settingsEvent.oldRotations = parser.oldRotations;
            ...
            <NitroCardContentView gap={3} className="text-white">
                <Column gap={1}>
                    ...
                    <Flex alignItems="center" gap={1}>
                        ...
                        <Text>{LocalizeText('memenu.settings.static.rotation')}</Text>
                    </Flex>
                     <Flex alignItems="center" gap={1}>
                        <input className="flash-form-check-input" type="checkbox" checked={ !!userSettings.wiredAnimations } onChange={ event => processAction('wired_animations', event.target.checked) } />
                        <Text>{LocalizeText('memenu.settings.wired.animations')}</Text>
                    </Flex>
                </Column>
                <Column>
