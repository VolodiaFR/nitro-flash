...
import { AboutAlertEvent } from '@nitrots/nitro-renderer/src/nitro/communication/messages/incoming/notifications/AboutAlertEvent';
...
const useNotificationState = () =>
{
    ...
    const customAlert2 = useCallback((
        message: string = null,
        type: string = null,
    ...
    ) =>
    {
        const alertItem = new NotificationAlertItem(
            null, // messages
            NotificationAlertType.ABOUT_ALERT,
            null, // clickUrl
            null, // clickUrlText
            null, // title
            null, // imageUrl
    ...
    useMessageEvent<AboutAlertEvent>(AboutAlertEvent, event =>
    {
        const parser = event.getParser();

        customAlert2(null, NotificationAlertType.ABOUT_ALERT, parser.username, parser.look, parser.version, parser.onlineUsers, parser.activeRooms, parser.catalogPages, parser.items, parser.furnis, parser.uptime, parser.ramUsage, parser.cpuCores, parser.totalMemory, parser.hotelName);
    });
