...
export class RoomMessageHandler extends Disposable
{
    ...
    public setConnection(connection: IConnection)
    {
        ...
        this._connection.addMessageEvent(new BulkHeightMapUpdateEvent(this.onBulkHeightMapUpdateEvent.bind(this)));
        ...
        this._connection.addMessageEvent(new BulkSlideUsersItemsEvent(this.onBulkSlideUsersItemsEvent.bind(this)));
        ...
        this._connection.addMessageEvent(new RoomUnitFocusEvent(this.onRoomUnitFocusEvent.bind(this)));
        ...
        this._connection.addMessageEvent(new PlayerOnClickThroughEvent(this.onPlayerOnClickThroughEvent.bind(this)));
        this._connection.addMessageEvent(new PlayerWalkKeysEvent(this.onPlayerWalkKeysEvent.bind(this)));
        this._connection.addMessageEvent(new PlayerVimKeysEvent(this.onPlayerVimKeysEvent.bind(this)));
    ...
    private onPlayerOnClickThroughEvent(event: PlayerOnClickThroughEvent): void
    {
        if (!event) return;
    ...
    private onPlayerWalkKeysEvent(event: PlayerWalkKeysEvent): void
    {
        if (!event) return;
        ...
        // Store walkkeys status globally for the toolbar to use
        (window as any).walkKeysEnabled = parser.isEnabled;
    }

    private onPlayerVimKeysEvent(event: PlayerVimKeysEvent): void
    {
        if (!event) return;
        ...
        // Store vim keys status globally for the toolbar to use
        (window as any).vimKeysEnabled = parser.isEnabled;
    }
    ...
    private onBulkHeightMapUpdateEvent(event: BulkHeightMapUpdateEvent): void
    {
        if (!(event instanceof BulkHeightMapUpdateEvent) || !event.connection || !this._roomCreator) return;
        ...
        // Update the heightmap with the stacking heights from the tiles
        for (const item of parser.items)
        {
            if (!item) continue;

            heightMap.setTileHeight(item.x, item.y, item.height);
        }

        this._roomCreator.refreshTileObjectMap(this._currentRoomId, 'RoomMessageHandler.onBulkHeightMapUpdateEvent()');
    }
    ...
    private onBulkUpdateFloorItemsEvent(event: BulkUpdateFloorItemsEvent): void
    {
        if (!(event instanceof BulkUpdateFloorItemsEvent) || !event.connection || !this._roomCreator) return;

        const items = event.getParser().items;

        if (!items || items.length === 0) return;

        // Procesar cada item del bulk update
    ...
    private onBulkSlideUsersItemsEvent(event: BulkSlideUsersItemsEvent): void
    {
        ...
        for (const slide of playerSlides) 
        {
            if (!slide) continue;

            // Usar rollRoomObjectFloor para animar el movimiento
            this._roomCreator.rollRoomObjectUser(
                this._currentRoomId,
                slide.roomIndex,
                slide.fromX,
                slide.fromY,
                parseFloat(slide.fromZ),
                slide.toX,
                slide.toY,
                parseFloat(slide.toZ),
                slide.fromStatus,
                slide.toStatus,
                slide.fromRotation,
                slide.toRotation
            );
            this._roomCreator.updateRoomObjectUserPosture(this._currentRoomId, slide.roomIndex, slide.toStatus);
        }
    }
    ...
    private onRoomUnitFocusEvent(event: RoomUnitFocusEvent): void
    {
        if (!(event instanceof RoomUnitFocusEvent) || !event.connection || !this._roomCreator) return;

        const parser = event.getParser();

        if (!parser) return;

        const alpha = Math.max(0, Math.min(255, parser.alpha));

        this._roomCreator.updateRoomObjectUserAction(this._currentRoomId, parser.unitId, RoomObjectVariable.FIGURE_FOCUS_ALPHA, alpha);
    }
