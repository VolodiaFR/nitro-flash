...
export class RoomMessageHandler extends Disposable {
    ...
    public setConnection(connection: IConnection) {
        ...
        this._connection.addMessageEvent(new RoomUnitStatusCustomEvent(this.onRoomUnitStatusCustomEvent.bind(this)));
    ...
    private onRoomUnitStatusCustomEvent(event: RoomUnitStatusCustomEvent): void {
        ...
        for (const status of statuses) {
            ...
            if (status.animationtime > 0 && status.didMove) {
                setTimeout(() => {
                    const finalLocation = new Vector3d(status.targetX, status.targetY, status.targetZ + height);
                    this._roomCreator.updateRoomObjectUserLocation(this._currentRoomId, status.id, finalLocation, null, status.canStandUp, height, direction, status.headDirection);
                }, status.animationtime);
            }
            ...
            if (status.animationtime > 0) {
                setTimeout(() => {
                    this._roomCreator.updateRoomObjectUserPosture(this._currentRoomId, status.id, RoomObjectVariable.STD, '');
                }, status.animationtime);
            }
        }
