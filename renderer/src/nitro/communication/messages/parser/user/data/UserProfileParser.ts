...
export class UserProfileParser implements IMessageParser
{
    ...
    // New field: total time played in seconds (added after secondsSinceLastVisit)
    this._timePlayedSeconds = wrapper.readInt();
        this._openProfileWindow = wrapper.readBoolean();
        // New optional field (server now sends VIP flag)
        try {
    ...
    public get timePlayedSeconds(): number
    {
