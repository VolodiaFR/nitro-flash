...
export const WiredActionSetFurniStateToView: FC<{}> = props => {
    ...
    return (
        <WiredActionBaseView requiresFurni={WiredFurniType.STUFF_SELECTION_OPTION_BY_ID} hasSpecialInput={true} save={save}>
            <Column gap={1}>
                <Text style={{marginLeft:"5px", marginBottom:"5px"}} bold>{LocalizeText('wiredfurni.params.conditions')}</Text>
                <Flex alignItems="center" gap={1}>
                    <input className="check-menu-wired" type="checkbox" id="stateFlag" checked={!!stateFlag} onChange={event => setStateFlag(event.target.checked ? 1 : 0)} />
                    <Text style={{ textIndent: "10px" }}>{LocalizeText('wiredfurni.params.condition.state')}</Text>
                </Flex>
                <Flex alignItems="center" gap={1}>
                    <input className="check-menu-wired" type="checkbox" id="directionFlag" checked={!!directionFlag} onChange={event => setDirectionFlag(event.target.checked ? 1 : 0)} />
                    <Text style={{ textIndent: "10px" }}>{LocalizeText('wiredfurni.params.condition.direction')}</Text>
                </Flex>
                <Flex alignItems="center" gap={1}>
                    <input className="check-menu-wired" type="checkbox" id="positionFlag" checked={!!positionFlag} onChange={event => setPositionFlag(event.target.checked ? 1 : 0)} />
                    <Text style={{ textIndent: "10px" }}>{LocalizeText('wiredfurni.params.condition.position')}</Text>
                </Flex>
                <Flex alignItems="center" gap={1}>
