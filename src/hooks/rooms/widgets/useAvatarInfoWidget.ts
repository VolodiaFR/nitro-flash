...
import { AvatarInfoFurni, AvatarInfoName, AvatarInfoPet, AvatarInfoRentableBot, AvatarInfoUser, AvatarInfoUtilities, CanManipulateFurniture, FurniCategory, GetRoomEngine, GetSessionDataManager, IAvatarInfo, IsOwnerOfFurniture, RoomWidgetUpdateRoomObjectEvent, SendMessageComposer, UseProductItem } from '../../../api';
...
import { setClearUserDisplayDataCallback, useVariableHighlight } from './variables/useVariableHighlight';
...
const useAvatarInfoWidgetState = () =>
{
    ...
    const [ variableBubbles, setVariableBubbles ] = useState<AvatarInfoName[]>([]);
    ...
    const [ blackjackSessions, setBlackjackSessions ] = useState<Map<number, any>>(new Map());
    const [ showBetDialog, setShowBetDialog ] = useState(false);
    const [ betSessionId, setBetSessionId ] = useState(-1);
    const { friends = [] } = useFriends();
    const { selectObjectForWired = null } = useWired();
    const { roomSession = null } = useRoom();
    const { activeHighlight } = useVariableHighlight();

    setClearUserDisplayDataCallback(() => {
        // Only clear if there's no active highlight session
        if (!activeHighlight) {
            setVariableBubbles([]);
        }
    });
    ...
    useMessageEvent<UserWithVariablesMessageEvent>(UserWithVariablesMessageEvent, event => {
        ...
        if (parser.assignments.length === 0) {
            // Only clear if there's no active highlight or if it's a different variable
            if (!activeHighlight || activeHighlight.variableName === parser.variableName) {
                setVariableBubbles([]);
            }
        } else {
            // Update existing bubbles instead of replacing them completely
            setVariableBubbles(prevBubbles => {
                // Keep bubbles that are not for this variable (if there are multiple variables being highlighted)
                const otherVariableBubbles = activeHighlight && activeHighlight.variableName !== parser.variableName
                    ? prevBubbles.filter(bubble => {
                        // This is a simple check - in a more complex system you might need to track which bubble belongs to which variable
                        return true; // For now, keep all other bubbles
                    })
                    : [];

                // Create new bubbles for the current variable
                const newBubbles: AvatarInfoName[] = [];
                parser.assignments.forEach(assignment => {
                    const userData = roomSession?.userDataManager?.getUserDataByIndex(assignment.roomIndex);
                    if(userData) {
                ...
                // Return the combination of other bubbles and new bubbles
                return [...otherVariableBubbles, ...newBubbles];
            });
        }
    });
    ...
    const removeBlackjackSession = (botId: number) =>
    {
        setBlackjackSessions(prev =>
        {
    ...
    {
        SendMessageComposer(new BlackjackBetMessageComposer(betSessionId, amount));
    ...
    ], event =>
    {
        ...
        if(event.category === RoomObjectCategory.UNIT)
        {
            ...
            index = variableBubbles.findIndex(bubble => (bubble.roomIndex === event.id));

            if(index > -1) setVariableBubbles(prevValue => prevValue.filter(bubble => (bubble.roomIndex !== event.id)));
            ...
            if(blackjackSessions.has(event.id))
            {
                setBlackjackSessions(prev =>
                {
                    const newMap = new Map(prev);
    ...
    useMessageEvent<BlackjackSessionStatusMessageEvent>(BlackjackSessionStatusMessageEvent, event =>
    {
        const parser = event.getParser();
        const data = JSON.parse(parser.data);
        const sessionData = { sessionId: parser.sessionId, ...data };
        const roomObjects = GetRoomEngine().getRoomObjects(roomSession.roomId, RoomObjectCategory.UNIT);
        let roomIndex = -1;
        for(const roomObject of roomObjects) {
            const userData = roomSession.userDataManager.getUserDataByIndex(roomObject.id);
            if(userData && userData.webID === parser.botId) {
                roomIndex = roomObject.id;
                break;
            }
        }
        if(roomIndex === -1) return;
        setBlackjackSessions(prev => new Map(prev).set(roomIndex, sessionData));
    });

    useMessageEvent<BlackjackBetPromptMessageEvent>(BlackjackBetPromptMessageEvent, event =>
    {
        const parser = event.getParser();
    ...
    // When opening an info panel (selecting furni/user) we previously cleared ALL overlays,
    // which also removed the user variable highlight bubbles. That made the highlight vanish
    // on any interaction. Keep variable bubbles while a highlight is active.
    useEffect(() =>
    {
        ...
        setActiveNameBubble(null);
        setNameBubbles([]);
        // Only clear variable bubbles if there is no active highlight session
        if(!activeHighlight) {
            setVariableBubbles([]);
        }
        setProductBubbles([]);
    }, [ avatarInfo, activeHighlight ]);
    ...
    return { avatarInfo, setAvatarInfo, activeNameBubble, setActiveNameBubble, nameBubbles, variableBubbles, productBubbles, confirmingProduct, blackjackSessions, isDecorating, setIsDecorating, removeNameBubble, removeProductBubble, updateConfirmingProduct, removeBlackjackSession, getObjectName, showBetDialog, betSessionId, handleBet, cancelBet };
}
